...
async function uploadSatelliteImage() {
    ...
    input.onchange = async () => {
        ...
        formData.append("satellite", "Sentinel-2");
        formData.append("latitude", "20.5937");
        formData.append("longitude", "78.9629");
        formData.append("resolution", "10");
        formData.append("cloud_cover", "5");

        try {
            const uploadRes = await fetch(`${SATELLITE_API_URL}/satellite/upload`, {
                method: "POST",
                body: formData
            });
            ...
            alert("Upload Success! Image ID: " + uploadData.id);

            const analyzeRes = await fetch(
                `${SATELLITE_API_URL}/satellite/analyze/${uploadData.id}`,
                {
                    method: "POST"
                }
            );
